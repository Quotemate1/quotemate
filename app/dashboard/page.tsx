    ...
    <div className="min-h-screen bg-stone-950">
      <nav className="border-b border-stone-800 bg-stone-900 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          {business ? (
            <span className="hidden md:inline text-sm text-stone-500 border-l border-stone-700 pl-4">{business.business_name}</span>
          ) : null}
        </div>
        <div className="flex items-center gap-2 md:gap-3">
          <a href="/referrals" className="px-4 py-2 bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/30 text-amber-400 rounded-full text-sm font-semibold transition-all">Refer Mates</a>
          <a href="/settings" className="px-4 py-2 bg-stone-800 hover:bg-stone-700 border border-stone-700 text-stone-300 rounded-full text-sm font-semibold transition-all">Settings</a>
          <span className="text-sm text-stone-500 hidden lg:inline">{user?.email}</span>
          <button onClick={handleSignOut} className="text-sm text-stone-500 hover:text-white transition-colors px-2">Sign out</button>
        </div>
      </nav>
      ...
      <main className="max-w-6xl mx-auto px-6 py-12">
        <div className="flex flex-wrap items-end justify-between gap-4 mb-10">
          ...
          <a href="/create-quote" className="px-6 py-3 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-full transition-colors shadow-lg shadow-emerald-500/20">+ New Quote</a>
        </div>
        ...
        {quotes.length === 0 ? (
          <div className="bg-stone-900 border border-stone-800 rounded-2xl p-12 text-center">
            ...
            <a href="/create-quote" className="inline-block px-6 py-3 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-full transition-colors">+ Create Your First Quote</a>
          </div>
        ) : (
          <div className="bg-stone-900 border border-stone-800 rounded-2xl overflow-hidden">
            ...
            {quotes.map((quote: any) => (
              <button key={quote.id} onClick={() => setSelectedQuote(quote)} className="w-full grid grid-cols-5 gap-4 px-6 py-4 border-b border-stone-800 hover:bg-stone-800/50 transition-colors items-center text-left">
                ...
                <span className={`text-xs px-3 py-1 rounded-full capitalize w-fit ${statusColor(quote.status)}`}>{statusLabel(quote.status)}</span>
      ...
      {selectedQuote ? (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-start justify-center p-6 z-50 overflow-y-auto" onClick={() => setSelectedQuote(null)}>
          <div className="bg-stone-900 border border-stone-700 rounded-2xl max-w-3xl w-full my-8" onClick={(e) => e.stopPropagation()}>
            <div className="border-b border-stone-800 px-6 py-4 flex items-center justify-between sticky top-0 bg-stone-900 z-10 rounded-t-2xl">
              <div>
              ...
              <div className="flex items-center gap-3">
                <span className={`text-xs px-3 py-1 rounded-full capitalize ${statusColor(selectedQuote.status)}`}>{statusLabel(selectedQuote.status)}</span>
                <button onClick={() => setSelectedQuote(null)} className="text-stone-400 hover:text-white text-2xl leading-none">x</button>
              </div>
            </div>
            ...
            {(selectedQuote.status === 'sent' || selectedQuote.status === 'opened' || selectedQuote.status === 'draft') ? (
              <div className="bg-stone-900 border-b border-stone-700 px-6 py-4">
                ...
                <div className="flex gap-3">
                  <button onClick={() => updateQuoteStatus(selectedQuote.id, 'accepted')} disabled={updatingStatus} className="flex-1 py-2 bg-emerald-500 hover:bg-emerald-600 text-white font-semibold rounded-full transition-colors disabled:opacity-50">Mark as Won</button>
                  <button onClick={() => updateQuoteStatus(selectedQuote.id, 'declined')} disabled={updatingStatus} className="flex-1 py-2 bg-stone-700 hover:bg-red-900 text-white font-semibold rounded-full transition-colors disabled:opacity-50">Mark as Lost</button>
                </div>
              </div>
            ) : null}
            ...
            {(selectedQuote.status === 'accepted' || selectedQuote.status === 'declined') ? (
              <div className={`border-b px-6 py-3 flex items-center justify-between ${selectedQuote.status === 'accepted' ? 'bg-emerald-950 border-emerald-900' : 'bg-red-950 border-red-900'}`}>
                <p className="text-sm font-semibold text-white">{selectedQuote.status === 'accepted' ? 'Job won!' : 'Job lost'}</p>
                <button onClick={() => updateQuoteStatus(selectedQuote.id, 'sent')} disabled={updatingStatus} className="text-xs text-stone-400 hover:text-white">Reset status</button>
              </div>
            ) : null}
            ...
            <div className="p-6">
              <div className="bg-white text-stone-900 rounded-lg p-8">
                ...
                <div className="mb-6">
                  ...
                  {selectedQuote.customers?.email ? <p className="text-stone-500 text-sm">{selectedQuote.customers.email}</p> : null}
                </div>
                {(() => {
                  const content = parseQuoteContent(selectedQuote.ai_generated_content)
                  if (!content) return <p className="text-stone-500 italic">Quote content unavailable</p>
                  return (
                    <>
                      {content.greeting ? <p className="mb-6 text-stone-700">{content.greeting}</p> : null}
                      {content.scopeOfWork ? (
                        <div className="mb-6">
                      ...
                      {content.inclusions && content.inclusions.length > 0 ? (
                        <div className="mb-6">
                          <h4 className="font-bold mb-2">Whats Included</h4>
                      ...
                      {content.validityPeriod ? <p className="text-stone-600 text-sm italic">Quote valid for {content.validityPeriod}</p> : null}
                      {content.closingMessage ? <p className="mt-4 text-stone-700">{content.closingMessage}</p> : null}
                    </>
                  )
                })()}
              </div>
