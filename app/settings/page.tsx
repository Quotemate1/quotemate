...
export default function SettingsPage() {
  ...
  const loadTemplate = async () => {
    ...
    if (!template) {
      alert('No template available for your trade type. Add items manually below.')
      return
    }

    const confirmed = confirm(
      `Load ${template.length} pre-built items for ${tradeKey}?\n\nThese are common Australian rates — you can edit or delete any after they're loaded.`
    )
    if (!confirmed) return
  ...
  return (
    <div className="min-h-screen bg-gray-950">
      ...
      <main className="max-w-3xl mx-auto px-6 py-12">
        ...
        <p className="text-gray-400 mb-10">Manage your saved line items — common things you charge for that auto-load on quotes.</p>
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <button
                  ...
                  className="px-5 py-2 bg-emerald-500 hover:bg-emerald-600 text-white font-semibold rounded transition-colors disabled:opacity-50"
                >
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-lg p-6 mb-6">
          ...
          <div className="flex gap-3 flex-wrap">
            <input
              ...
              className="flex-1 min-w-[200px] px-4 py-3 bg-gray-800 border border-gray-700 rounded text-white placeholder-gray-500 focus:outline-none focus:border-emerald-500"
              placeholder="e.g. Service call (first hour)"
            />
            <input
              type="number"
              value={newAmount}
            ...
            <button
              ...
              className="px-6 py-3 bg-emerald-500 hover:bg-emerald-600 text-white font-semibold rounded transition-colors disabled:opacity-30"
            >
              {adding ? 'Adding...' : 'Add'}
            </button>
          </div>
        ...
        {items.length === 0 ? (
          <div className="bg-gray-900 border border-gray-800 rounded-lg p-12 text-center">
            ...
            <p className="text-gray-500 text-sm">{hasTemplate ? 'Load a template above or add items manually.' : 'Add items above to speed up your quotes.'}</p>
          </div>
        ) : (
              ...
              <div key={item.id} className="border-b border-gray-800 last:border-b-0">
                {editingId === item.id ? (
                  <div className="p-4 bg-gray-800">
                    <div className="flex gap-3 flex-wrap mb-3">
                      <input
                        ...
                        className="flex-1 min-w-[200px] px-3 py-2 bg-gray-900 border border-emerald-500 rounded text-white focus:outline-none"
                      ...
                      <input
                        ...
                        className="w-32 px-3 py-2 bg-gray-900 border border-emerald-500 rounded text-white focus:outline-none"
                        placeholder="$ Amount"
                      />
                    </div>
                    <div className="flex gap-2">
                      <button
                        ...
                        disabled={!editDesc.trim() || !editAmount}
                        className="px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-semibold rounded transition-colors disabled:opacity-30"
                      >
                        ✓ Save
                      </button>
                      <button
                  ...
                  <div
                    ...
                    className="flex items-center justify-between px-6 py-4 hover:bg-gray-800 cursor-pointer transition-colors group"
                  >
                    <div className="flex-1">
                      <p className="text-white">{item.description}</p>
                      <p className="text-xs text-gray-600 opacity-0 group-hover:opacity-100 transition-opacity">Click to edit</p>
                    </div>
